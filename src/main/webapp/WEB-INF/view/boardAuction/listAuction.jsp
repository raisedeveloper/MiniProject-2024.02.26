						...
						<td style="width: 52%; text-align: left">
							<h3>
								<strong class="me-5">역경매 항목</strong> <span
									style="font-size: 16px"><a href="/mp/1.mini/board/insertAuction"><i
										class="fa-solid fa-pen-to-square"></i> 글 쓰기</a></span>
							</h3>
						</td>
						<td style="width: 16%"><select class="form-control"	id="field">
								<option value="applTime" ${field eq 'applTime' ? 'selected' : ''}>신청 시간</option>
								<option value="nickName" ${field eq 'nickName' ? 'selected' : ''}>닉네임</option>
								<option value="processTitle" ${field eq 'processTitle' ? 'selected' : ''}>제목</option>
								<option value="avgPrice" ${field eq 'avgPrice' ? 'selected' : ''}>평균 가격</option>
								<option value="numOfCompany" ${field eq 'numOfCompany' ? 'selected' : ''}>업체 수</option>
								<option value="process" ${field eq 'process' ? 'selected' : ''}>진행 사항</option>
